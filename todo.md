...
- [ ] **Promote `.claude/mcp_servers.json.example` to the real `.claude/mcp_servers.json`** — fill in the RHEL server's real hostname (`https://`, once TLS is live) and commit it. No secrets in this file (just hostnames/ports), so committing the real version is safe and gives engineers zero-config setup via sparse-checkout + `git pull`.
