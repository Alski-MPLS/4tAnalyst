...
**Zone name mapping:** The zone name returned by `/check-policy` is the 4THealth policy zone name — this may differ from the interface or zone name shown in FortiManager. Refer to `docs/zone-name-mapping.md` (work in progress) to translate between policy zones and FortiManager zone names before building rules. Flag requests where you are unsure and verify manually until the table is complete.
...
**Zone name translation:** 4THealth uses policy zone names (e.g., `OT-PROD`, `CIP-HIGH`). FortiManager uses ADOM-scoped zone names. These may differ. When building rules, use `docs/zone-name-mapping.md` to translate between policy zone names and FortiManager zone names. Until that table is complete, verify zone names manually against the FortiManager GUI.
