...
After running it, open `device_zone_map.yaml` and fill in the `policy_zone` value for each interface. Use `check_ip_traffic` or `get_zones()` to confirm the exact 4THealth zone name (names must match exactly).
...
```
FGT-OT-SITE01,port3,OT_LAN,NETZONE OT-All,Primary OT LAN segment
```
...
Zone names must match exactly. Use `get_zones()` for the canonical list, or look up a specific IP:
