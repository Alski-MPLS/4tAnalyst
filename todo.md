...
- [ ] **TLS** — static-bearer auth is now enforced on the single port 8000 (2026-07-04), but the transport is plain HTTP until TLS is actually turned on for a real deployment. **2026-08-08: no longer blocked on standing up nginx** — `fwanalyst_server/__main__.py` now supports direct uvicorn TLS via `FW_ANALYST_SSL_CERTFILE`/`FW_ANALYST_SSL_KEYFILE` (or `credentials.yaml` `server.ssl_certfile`/`ssl_keyfile`), documented in `docs/tls-setup.md`. Still open: pick direct-uvicorn or nginx for the actual central-server deployment and enable it before any real change request data flows through the system, especially in a regulated environment (NERC CIP, HIPAA, SOX, etc.).
...
- [x] **Create `.claude/mcp_servers.json.example`** — added; previously referenced by `docs/configuration.md` as "the example" but never actually existed in the repo. **Superseded 2026-08-08**: `.claude/mcp_servers.json.example` matched no path Claude Code actually reads; replaced by a root-level `.mcp.json.example` using `${FW_ANALYST_CLIENT_TOKEN}` env expansion. `.mcp.json` itself is now explicitly gitignored (the docs previously claimed this but it wasn't actually the case).
- [ ] **Promote `.mcp.json.example` to a committed `.mcp.json`** — fill in the RHEL server's real hostname (`https://`, once TLS is live) and commit it. The token is `${FW_ANALYST_CLIENT_TOKEN}` (env-var expansion, not a literal value), so the file itself carries no secret and never will — committing the real version is safe and gives engineers zero-config setup via sparse-checkout + `git pull`. (Note: `.mcp.json` is gitignored as defense in depth — this would need an explicit `!.mcp.json` exception or committing under a different tracked name.)
...
- [ ] **nginx / reverse proxy config, or the direct-uvicorn TLS alternative** — moved to blocking list above; must have TLS before production. Direct uvicorn TLS (`FW_ANALYST_SSL_CERTFILE`/`FW_ANALYST_SSL_KEYFILE`) is available now and needs no reverse proxy — see `docs/tls-setup.md` for when to prefer it over nginx.
