...
**Firewall credentials live only on the central server.** Engineers never need direct API access to FortiManager. The five per-domain packages still exist as code (and run individually over stdio for development), but production serves one authenticated endpoint instead of five unauthenticated SSE ports.

### planner/ — the deterministic core

`plan_change(src, dst, service, firewalls, justification, ticket_id, src_group, dst_group)` performs, in tested code. `src`/`dst`/`service` each accept a single value, a comma-separated string, or a list — one call plans **one consolidated policy per firewall** covering every combination:

1. **Zone verdict** — live 4THealth query per src×dst×service combination (`fetch.py`); API failure raises a typed `PlannerDataError`, never silently treated as "no result". IPs 4THealth cannot resolve default to the catch-all **Internet** zone (with an explanatory note) and the verdict is re-derived from the live policy table — the Internet zone is always critical risk with the internet-inbound/outbound logging profile. Any UNKNOWN combination makes the whole request UNKNOWN (no CLI); mixed ALLOWED+BLOCKED verdicts raise an error telling the engineer to split the request — a single rule must never carry both.
...
6. **Standards** (`standards.py`) — risk level (unknown zones fail safe to critical), logging profile, approval chain — the decision rules previously encoded as prose in SKILL.md, now unit-tested code.
...
Engineer: /analyze-request
    ...
    business justification, firewalls (DEVICE:ADOM), ticket
        ...
        plan_change (ONE tool call for the whole request)
                │
        planner/engine.py  (deterministic)
...
| Planner runs without an LLM (`python -m planner`) | Compliance can approve the deterministic tool independently of the AI inference path; also the correct correctness posture |
